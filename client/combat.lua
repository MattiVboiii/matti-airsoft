...
local function RebuildTrackedWeaponHashes()
	trackedWeaponHashes = {}
...
function Combat.TrackDamage()
	AddEventHandler("gameEventTriggered", function(event, data)
		...
		if Interaction.ShouldBlockCombatBetween(attackerServerId, GetPlayerServerId(PlayerId())) then
			return
		end

		if not Interaction.IsPlayerInMatch(attackerServerId) then
			return
		end
		...
		if Config.Debug then
			print(
				" Damage detected from player server ID: "
					.. tostring(attackerServerId)
					.. " with event weapon hash: "
					.. tostring(eventWeaponHash)
			)
		end
		...
		if trackedWeaponHash then
			...
			if Config.Debug then
				print(
					" Tracked airsoft hit from player server ID: "
						.. tostring(attackerServerId)
						.. " with weapon hash: "
						.. tostring(trackedWeaponHash)
				)
			end
		elseif Config.Debug then
			print(
				" Damage ignored for scoreboard (non-airsoft or unresolved weapon). Event weapon hash: "
					.. tostring(eventWeaponHash)
			)
		end
	end)
					...
					if Config.Debug then
						...
						print(
							" Player was hit ("
								.. stunStatus
								.. "). Cause hash: "
								.. tostring(causeOfDeath)
								.. ", Killer Ped: "
								.. tostring(killerPed)
								.. ", Killer server ID: "
								.. tostring(killerId)
								.. ", Counted on scoreboard: true, Credited killer: "
								.. tostring(shouldCreditKiller)
						)
					end

					TriggerServerEvent("matti-airsoft:playerWasHit", shouldCreditKiller)
