...
- Added a new shared helper module at `shared/utils.lua` and wired it into `fxmanifest.lua`.
- Removed duplicated game-mode parsing/normalization logic from both client and server modules.
- Consolidated menu input handling so `qb-input` and `ox_lib` prompts are routed through one abstraction.
- Unified item amount validation for server item events and moved max amount cap to `Config.MaxItemEventAmount`.
- Hardened framework and timer handling paths with safer numeric normalization and nil checks.
- Improved leaderboard/team mode robustness and ensured missing stat entries are initialized before mutation.
- Applied NUI cleanup refactors (theme warning logs, HUD reset helper, scrollbar fallback, accessibility attributes).

### Notes

- This release is a behavior-preserving internal cleanup focused on maintainability and safety.
- Resource version bumped from `2.0.2` to `2.0.3`.
